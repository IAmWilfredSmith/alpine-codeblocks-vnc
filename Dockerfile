####################################################################################################################################
#                                                                                                                                  #
# Dockerfile - Docker container definition for wilfredsmith/alpine-codeblocks-vnc                                     [Dockerfile] #
#                                                                                                                                  #
...
RUN echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>" > /etc/xdg/openbox/menu.xml && \
    ...
    echo "      <item label=\"code:Blocks\">" >> /etc/xdg/openbox/menu.xml && \
    echo "         <action name=\"Execute\">" >> /etc/xdg/openbox/menu.xml && \
    echo "            <command>/usr/local/bin/codeblocks</command>" >> /etc/xdg/openbox/menu.xml && \
...
RUN apk add subversion libtool m4 automake wxgtk2.8-dev autoconf gcc g++ hunspell hunspell-dev gamin-dev make zip boost-dev
RUN cd /tmp && \
    svn checkout http://svn.code.sf.net/p/codeblocks/code/trunk codeBlocks && \
    cd codeBlocks && \
    ./bootstrap && \
    ./configure --with-contrib-plugins=all --disable-dependency-tracking && \
    make -j 7 && \
    make install
